...
**Important note** : the `hack/app_sre_pr_check.sh` script is ensuring the committed templates are up-to-date and it is validating all generated files are up-to-date as well.

To do so, it is deleting `generated_content` and all files starting with `50_GENERATED`.
The second condition is to ensure the generated syncset (such as the content for the oauth template or the CMO files) are cleaned up as well. If you have new syncset requiring generate, ensure to follow the naming convention or update the `hack/app_sre_pr_check.sh` to cleanup your new files as well.
...
## How to handle different flavours for the same resources

In case you need to have almost identical versions for a given resource, those can be generated by a script and called in the Makefile in order to ensure the common parts remains in sync between the different flavours. An example of this can found in `scripts/generate-cmo-config.py` which is managing the CMO resources.

**Note** : In the future, we should aim at implementing a unique script and defining the customization in the `config.yaml` for the syncset to ease maintenance. A OSD JIRA card will be created for this.
