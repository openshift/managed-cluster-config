...
- Policies requires to be deploy in namespaces, so if the new policies don't belong to `openshift-managed-rbac-config`, create a manifest to create a new namespace. Example: `./deploy/acm-policies/00-openshift-managed-rbac-policies.Namespace.yaml`. After, add the new namespace parameter in `/script/policy-paths.yaml` and `/script/generate-policy-config.py`.
