...
def add_resource_to_deploy(directory):
    ...
    target_directory = os.path.join("./generated_deploy", dir)

    if not os.path.isdir(target_directory):
        os.makedirs(target_directory, 0o755)

    # copying only files to ensure we take the correct action for subdirs which are independant resources
    for file in os.listdir(directory):
        filename = os.path.join(directory, file)
        if os.path.isfile(filename):
            shutil.copy2(filename, re.sub('deploy/','generated_deploy/', filename ))
...
if __name__ == '__main__':
    ...
    for dirpath in sorted(dirpaths, key=str.casefold):
        ...
        if "selectorSyncSet" in config or "direct" in config:
            #if deploymentMode has SSS or Direct, copy that dir to /generated_deploy
