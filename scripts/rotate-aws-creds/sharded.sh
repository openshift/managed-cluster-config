...
ACCESS_KEY_ID=$(KUBECONFIG=${SHARD_KUBECONFIG} ${OC_CMD} get secrets -n aws-account-operator "${SRE_SECRET_NAME}" -o json | jq -r '.data.aws_access_key_id' | base64 -d)
SECRET_ACCESS_KEY=$(KUBECONFIG=${SHARD_KUBECONFIG} ${OC_CMD} get secrets -n aws-account-operator "${SRE_SECRET_NAME}" -o json | jq -r '.data.aws_secret_access_key' | base64 -d)
