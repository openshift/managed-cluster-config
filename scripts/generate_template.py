...
skip_file_patterns = [
    re.compile(r".*50-GENERATED-hypershift-splunk-audit-exporter.Policy.yaml$")
]
...
def get_all_yaml_obj(file_paths):
    ...
    for file in file_paths:
        for skip_file_pattern in skip_file_patterns:
            if skip_file_pattern.match(file):
                print("Skipping file as configured: ", file)
                skip = True
        if skip:
            continue
...
if __name__ == '__main__':
    ...
    for dirpath in sorted(dirpaths, key=str.casefold):
        ...
        elif deploymentMode == "Policy":
            # The file will be generated by generate-policy-config.py and picked
            # up from the `acm-policies` directory, so no need to add it twice.
            continue
