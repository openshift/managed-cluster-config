...
| SRE or ACS eng | rhacs-observability  | grafana-admin-credentials | SREs and ACS engineers need to login into Grafana as administrators to tweak dashboards. Changes to dashboards are temporary, as they are ultimately reconciled by [RHACS Observability Resources Repository](https://github.com/stackrox/rhacs-observability-resources). But, it's next-to-impossible to edit the grafana JSON dashboards manually (it's extremely verbose). The workflow includes: logging in as a Grafana administrator (hence the need to read this secret), modify the dashboards manually through ClickOps, export them as json, and issue a MR to the observability repository. |
